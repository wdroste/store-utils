...
public class StoreComparer {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.err.println(
                    "Usage: StoreComparer source target [rel,types,to,ignore] [properties,to,ignore]");
            return;
        }
        ...
        System.out.printf(
                "Copying from %s to %s ingoring rel-types %s ignoring properties %s %n",
    ...
    private static void compareStore(
            String sourceDir,
            String targetDir,
        ...
        if (!target.exists())
            throw new IllegalArgumentException("Target Directory does not exists " + target);
        if (!source.exists())
            throw new IllegalArgumentException("Source Database does not exist " + source);

        GraphDatabaseService targetDb =
                new GraphDatabaseFactory()
                        .newEmbeddedDatabaseBuilder(target)
                        .setConfig(config())
                        .newGraphDatabase();
        GraphDatabaseService sourceDb =
                new GraphDatabaseFactory()
                        .newEmbeddedDatabaseBuilder(source)
                        .setConfig(config())
                        .newGraphDatabase();
    ...
    private static Statistics count(
            GraphDatabaseService db, Set<String> ignoreRelTypes, Set<String> ignoreProperties) {
    ...
    private static Relationship getTargetRel(
            GraphDatabaseService gdb, long startNodeId, long endNodeId, RelationshipType relType) {
    ...
    private static void compareProperties(
            PropertyContainer pc1, PropertyContainer pc2, Set<String> ignoreProperties) {
        final Collection<String> keys1 =
                Iterables.addToCollection(pc1.getPropertyKeys(), new HashSet<String>());
        final Collection<String> keys2 =
                Iterables.addToCollection(pc2.getPropertyKeys(), new HashSet<String>());
        keys2.removeAll(ignoreProperties);
        keys1.removeAll(ignoreProperties);
        if (!keys1.equals(keys2)) {
            System.err.println(
                    "On " + pc1 + " != " + pc2 + " properties mismatch " + keys1 + " != " + keys2);
        }
    ...
    private static Map<String, Object> getProperties(
            PropertyContainer pc, Set<String> ignoreProperties) {
    ...
    private static class Statistics {
        ...
        public boolean equals(Object o) {
            ...
            if (nodeProperties != that.nodeProperties) {
                System.err.println(
                        "nodeProperties " + nodeProperties + " != " + that.nodeProperties);
                return false;
            }
            if (relationshipCount != that.relationshipCount) {
                System.err.println(
                        "relationshipCount " + relationshipCount + " != " + that.relationshipCount);
                return false;
            }
        ...
        public String toString() {
            return String.format(
                    "Statistics{nodes=%d, nodeProperties=%d, relationshipCount=%d, relationshipPropertyCount=%d}",
                    nodeCount, nodeProperties, relationshipCount, relationshipPropertyCount);
        }
    }
}
