...
public class BucketBuilder {
    ...
    public static List<Bucket> build(List<Pair<IndexData, Long>> pairs) {
        ...
        for (Pair<IndexData, Long> pair : pairs) {
            val size = toSize(pair.getValue());
            val l = map.computeIfAbsent(size, na -> new ArrayList<>());
            l.add(pair.getKey());
        }
    ...
    static Stream<Bucket> toBuckets(Size size, List<IndexData> values) {
        return Lists.partition(values, toBatchSize(size)).stream().map(b -> toBucket(size, b));
    }

    static Bucket toBucket(Size size, List<IndexData> values) {
        val batch = IndexBatch.builder().indexes(values).build();
