...
public class StoreCopyCLI implements Runnable {
    ...
    static void usage(CommandLine commandLine) {
        ...
        System.out.println("\nConfiguration Help: ");
        System.out.println(
                "  deleteRelationshipsWithType - delete relationships between nodes with the specified types.");
        System.out.println(
    ...
    public void run() {
        ...
        if (!configuration.isFile()) {
            throw new IllegalArgumentException(
                    "Configuration file does not exist " + configuration);
        }
        // make sure there's an existing 'store'
        if (!(new File(source, "neostore").isFile())) {
            throw new IllegalArgumentException(
                    "Invalid path '" + source + "' must contain at least 'neostore' file.");
        }
        ...
        System.out.printf(
                "Copying from %s to %s, with configuration: \n%s", source, target, configuration);
        System.out.println(gson.toJson(cfg));
